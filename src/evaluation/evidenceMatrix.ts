...
export const POST_R1_EVIDENCE_MATRIX: EvidenceMatrixEntry[] = [
  ...
  { invariantId: "context.manifest-privacy", claim: "Context provenance records allowlisted metadata without prompt, memory, path, or secret content", command: "npm run test:context-governance", evidenceFile: "src/scripts/test-context-governance.ts, src/context/provenance/manifest.ts" },
  { invariantId: "context.budget-accounting", claim: "Loaded context sources have deterministic category and token estimates without claiming provider billing", command: "npm run test:context-governance", evidenceFile: "src/scripts/test-context-governance.ts, src/context/provenance/types.ts" },
  { invariantId: "memory.path-isolation", claim: "Memory writes and archives reject traversal, reserved paths, root links, file links, and trash links", command: "npm run test:memory-governance", evidenceFile: "src/scripts/test-memory-governance.ts, src/context/memory/governance.ts" },
  { invariantId: "memory.revision-conflict", claim: "Existing memory cannot be silently overwritten or deleted without the current revision", command: "npm run test:memory-governance", evidenceFile: "src/scripts/test-memory-governance.ts, src/context/memory/governance.ts" },
  { invariantId: "memory.expiry-recoverable-delete", claim: "Expired memory is classified stale and deletion archives recoverable content outside the active index", command: "npm run test:memory-governance", evidenceFile: "src/scripts/test-memory-governance.ts, src/tools/memoryDeleteTool.ts" },
  { invariantId: "compaction.invariant-retention", claim: "Full compaction fails closed when required task constraints are absent from the summary", command: "npm run test:context-governance", evidenceFile: "src/scripts/test-context-governance.ts, src/context/compactionInvariants.ts" },
];
