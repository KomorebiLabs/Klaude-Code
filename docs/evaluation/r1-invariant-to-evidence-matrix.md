# R1 Invariant-to-Evidence Matrix（PR-09 最终版）
...
| `sandbox.permission-no-upgrade` | Sandbox 可用性和包装决策不改变 PR-07 的 deny/ask 授权结果；不支持平台显式降级 | `npm run test:external-safety && npm run test:tool-permission-contract` | `src/scripts/test-external-safety-contract.ts`, `src/scripts/test-tool-permission-contract.ts` |
...
统一门禁：`npm run verify:core`。它还会运行 `src/scripts/verify-r1-release.ts`，以单次 Fake Provider Trial 验证真实 `query → permission → tool → query completion` 调用链，并输出 25 条矩阵映射与 4 条端到端断言。该 Trial 只证明当前提交的确定性行为，不代表真实模型或跨仓库成功率。
