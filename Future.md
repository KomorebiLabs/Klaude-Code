...
Future.md 是长期方向和问题判断，不是当前执行看板。它只记录经过代码、提交、测试或明确决策支撑的内容。未来修改本文件时，必须把“事实变化”和“观点变化”分开。

### 8.1 哪些情况必须更新

出现以下任一情况时，应在同一个功能 PR 中同步更新本文件，或在 PR body 中明确说明为什么不更新：

- 某个候选阶段从 deferred 变成 authorized、active、completed 或 blocked；
- 实际实现改变了本文件描述的调用链、数据边界、权限语义或兼容行为；
- 新增了重要的安全、隐私、可靠性或发布限制；
- 原有问题已经被真实证据关闭，或发现原问题判断不准确；
- 新增 Provider、Extension、Storage、Evaluation 或 Release 能力；
- 旧的未来计划因依赖、成本、时间或真实 Bad Case 而调整顺序；
- 发现 README、Roadmap、Evaluation、Dev Doc 之间出现事实冲突。

### 8.2 修改步骤

1. 在代码或合并提交中找到事实来源；如果没有事实来源，先把内容写成“假设/待验证”，不要写成结论；
2. 更新 docs/PROJECT-SNAPSHOT.md 中的当前状态、证据和限制；
3. 更新本文件中的对应问题、优先级或路线；
4. 如果进入执行窗口，再更新 Future Doing.md 中对应工作包的状态；
5. 在 Dev Doc 中记录实际调用链、变更、验证、失败和剩余限制；
6. 更新 README 或 docs/README.md 的入口链接和状态摘要；
7. 运行构建、相关测试、链接检查和必要的 GitNexus 变更检查；
8. 在 PR body 中写明“更新前事实 → 新事实 → 证据 → 仍未完成边界”。

### 8.3 禁止的修改方式

- 不能因为写进了 Future Doing.md 就把候选任务标成已完成；
- 不能使用“企业级”“生产就绪”“高成功率”等没有指标和样本支持的结论；
- 不能用一次成功 Demo 替代回归测试、失败证据和限制说明；
- 不能把继承 Foundation Track 的能力描述成当前维护者从零实现；
- 不能提前为远期阶段固定会变化的文件名、符号或数据库方案；
- 不能在封箱状态下因文档存在而自动启动任何功能开发。

## 9. 未来工作的决策矩阵

恢复开发时，先根据目标选择路线，不要默认按 PR 编号连续执行：

| 目标 | 首选工作包 | 不能省略的证据 | 不适合先做 |
|---|---|---|---|
| 解决并行任务失控 | PR-12 → PR-13 | 父子状态、Worktree baseline、失败交接、恢复结果 | 先做 UI 美化 |
| 对外展示/安装 | PR-17 | 安装、升级、迁移、回滚、跨平台冒烟 | 先做插件市场 |
| 证明真实模型行为 | PR-16 | 任务集、样本、成本、失败分类、环境记录 | 只展示成功截图 |
| 提升长上下文可解释性 | Context Decision Replay | 裁剪决策、预算摘要、可重复装配、隐私证据 | 直接上向量数据库 |
| 提升长期记忆质量 | Memory Quality | 冲突检测、索引重建、并发写、删除恢复 | 静默自动合并事实 |
| 开放第三方能力 | PR-15 | Capability、信任、隔离、版本兼容、失败降级 | 无权限模型的扩展生态 |

如果同一时间只能完成一个阶段，优先选择“能关闭真实 Bad Case 且能产生可复述证据”的阶段，而不是选择代码量最大或名词最热门的阶段。

## 10. 当前封箱版本的重启条件

以下条件全部满足后，才适合从封箱转为新的开发窗口：

- 有明确的用户问题、面试展示目标或维护缺陷；
- 能用一句话说明本阶段的非目标；
- 最新 github/main 已同步核对，且没有误吸收用户未提交文件；
- npm run verify:core 已有新的基线结果；
- 目标阶段的前置 PR、Dev Doc、Spec 和限制已读过；
- 已决定一个、且只有一个当前 Stage；
- 已写出带验收标准、验证命令和停止条件的 JIT Plan；
- 用户明确授权开始该 Stage。

如果缺少其中任一条件，停留在维护暂停或继续做只读调查，不进入运行时代码修改。

## 11. 与 Future Doing.md 的关系

- docs/PROJECT-SNAPSHOT.md：现在是什么，当前有哪些证据和限制；
- Future.md：为什么要继续、问题是什么、哪些方向值得做；
- Future Doing.md：恢复开发后具体如何拆任务、执行什么操作、用什么证据收口；
- MainTask.md：长期 Enterprise Harness 方向和定义；
- 阶段计划/JIT Plan：当前唯一授权 Stage 的实施细节；
- Dev Doc：某个已执行阶段的真实实施记录。

未来 Agent 应按这个顺序使用：先读 Snapshot 判断状态，再读 Future 判断价值和优先级，最后只抽取一个 Future Doing.md 工作包写成新的 JIT Plan。不得直接把整份未来计划一次性装入上下文并开始执行。
